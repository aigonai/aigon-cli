...
def test_requests_shim_functions_signature():
    ...
    expected_public_functions = {"get", "post", "put", "patch", "delete"}

    expected_private_functions = {"_build_url", "_make_request"}

    # Get all functions
    actual_public_functions = set()
    actual_private_functions = set()

    for name, _obj in inspect.getmembers(requests_shim, inspect.isfunction):
...
def test_requests_shim_classes_signature():
    ...
    actual_classes = set()
    for name, _obj in inspect.getmembers(requests_shim, inspect.isclass):
