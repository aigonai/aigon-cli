...
def do_global_search(client: AigonClient,
                     query: str,
                     scope: str = "all",
                     grouping: str = "merged",
                     note_type: str = "all",
...
def register_search_commands(subparsers) -> None:
    ...
    # Scope options
    search_parser.add_argument('--scope', '-s', default='all',
                              help='Comma-separated: notes,system,attachments,files,all (default: all)')

    # Grouping and format options
    search_parser.add_argument('--grouping', '-g', choices=['merged', 'grouped'], default='merged',
                              help='Result grouping: merged (single list) or grouped (by type)')
    search_parser.add_argument('--format', '-f', choices=['json', 'llm'], default='llm',
                              help='Output format: json or llm (default: llm)')

    # Note filters
    search_parser.add_argument('--note-type', choices=['user', 'system', 'ephemeral', 'all'], default='all',
                              help='Note type filter (default: all)')

    # File filters
    search_parser.add_argument('--file-versions', choices=['latest', 'all'], default='latest',
                              help='File versions: latest or all (default: latest)')

    # Time filters
    search_parser.add_argument('--days', type=float, default=None,
                              help='Search last N days (shortcut for --from N)')
    search_parser.add_argument('--from', dest='time_from', type=float, default=None,
                              help='Days back to start search')
    search_parser.add_argument('--to', dest='time_to', type=float, default=0.0,
                              help='Days back to end search (default: 0 = now)')
    search_parser.add_argument('--recent', action='store_true',
                              help='Last 24 hours (shortcut for --days 1)')
    search_parser.add_argument('--week', action='store_true',
                              help='Last 7 days (shortcut for --days 7)')

    # Status filters
    search_parser.add_argument('--export-status', choices=['all', 'exported', 'unexported'], default='all',
                              help='Export status filter')
    search_parser.add_argument('--processed-status', choices=['all', 'processed', 'unprocessed'], default='all',
                              help='Processed status filter')
    search_parser.add_argument('--deleted-status', choices=['active', 'deleted', 'all'], default='active',
                              help='Deleted status filter (default: active)')

    # Agent filter
    search_parser.add_argument('--agent', default=None,
                              help='Filter notes by agent (notetaker, wellness, coach, etc.)')
