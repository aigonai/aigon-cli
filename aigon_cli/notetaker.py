...
def _format_note_llm(note: dict, show_user_id: bool = False) -> str:
    ...
    lines.extend(["content: ---", content, "---", "--- END NOTE ---"])
...
def mailbox_reply(
    client: AigonClient, unique_id: str, text: str, as_markdown: bool = False, delay: int = 5, bcc: list = None
) -> None:
...
def mailbox_send(
    client: AigonClient, to: str, subject: str, text: str, as_markdown: bool = False, delay: int = 5, bcc: list = None
) -> None:
    """Send a new email.
...
def mark_notes(
    ...
    """
    try:
        result = client.mark_notes(unique_ids=unique_ids, processed=processed, exported=exported, deleted=deleted)
...
def update_notes(
    ...
    summary: str = None,
    metadata_set: str = None,
    metadata_merge: str = None,
    metadata_remove_keys: str = None,
    delegates_add: str = None,
    delegates_remove: str = None,
    output_format: str = "llm",
) -> None:
...
def _attachment_download_filename(
    note_uid: str, index: int, attachment_uid: str, content_type: str, original_filename: str
) -> str:
...
def get_attachment(client: AigonClient, note_id: str, filename: str = None, download_directory: str = None) -> None:
    ...
    try:
        ...
        notes = client.get_notes_by_ids([note_id], context_before=0, context_after=0, with_attachments=True)
        ...
        if filename is not None and filename.isdigit():
            ...
            if selected_att is None:
                ...
                print(
                    f"Available: {', '.join(a.get('original_filename') or a.get('filename', '?') for a in attachments)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
        ...
        if not is_displayable and download_directory is None:
            print(
                f"Error: Attachment has MIME type '{mime_type}' which cannot be displayed to stdout.", file=sys.stderr
            )
...
def save_report(
    ...
    try:
        ...
        if output_format == "json":
            ...
            print(
                f"Saved report [{unique_id}]: {filename} (agent={agent_out}, type={report_type_out}, date={report_date})"
            )
...
def register_notetaker_commands(subparsers):
    ...
    search_parser.add_argument(
        "--type", dest="content_type", choices=["text", "audio", "image"], help="Filter by content type"
    )
    search_parser.add_argument(
        "--limit", type=int, default=None, help="Maximum results (default: 10 for llm/json, 100 for snippet/summary)"
    )
    search_parser.add_argument(
        "--format",
        choices=["json", "llm", "snippet", "summary", "full"],
        default=None,
    ...
    search_parser.add_argument(
        "--from", dest="from_days", type=float, help="Days back to start search (e.g., --from 7)"
    )
    search_parser.add_argument(
        "--to", dest="to_days", type=float, default=0.0, help="Days back to end search (default: 0.0 = now)"
    )
    search_parser.add_argument(
        "--days", dest="days_back", type=float, help="Search last N days (shortcut for --from N --to 0)"
    )
    search_parser.add_argument("--recent", action="store_true", help="Search last day (shortcut for --from 1)")
    search_parser.add_argument("--week", action="store_true", help="Search last week (shortcut for --from 7)")
    search_parser.add_argument("--forever", action="store_true", help="All time (no time limit)")

    # Time filtering options (absolute - ISO dates)
    search_parser.add_argument(
        "--start", dest="start_date", help="Absolute start date (ISO format: 2025-12-01 or Unix timestamp)"
    )
    search_parser.add_argument(
        "--end", dest="end_date", help="Absolute end date (ISO format: 2025-12-10 or Unix timestamp)"
    )
    search_parser.add_argument(
        "--time-field",
        dest="time_field",
    ...
    search_parser.add_argument("--exported", action="store_true", help="Only exported notes")
    search_parser.add_argument("--unexported", action="store_true", help="Only unexported notes")
    search_parser.add_argument("--processed", action="store_true", help="Only processed notes")
    search_parser.add_argument("--unprocessed", action="store_true", help="Only unprocessed notes")
    search_parser.add_argument("--new", action="store_true", help="New notes (unexported and unprocessed)")

    # Content control options
    search_parser.add_argument(
        "--preview", type=int, dest="preview_length", help="Show only first N characters of content"
    )
    search_parser.add_argument("--titles-only", action="store_true", help="Show only metadata, no content")

    # Search filters: user/system notes and attachments
    search_parser.add_argument(
        "--user", dest="search_user", action="store_true", default=True, help="Include user notes (default: true)"
    )
    search_parser.add_argument("--no-user", dest="search_user", action="store_false", help="Exclude user notes")
    search_parser.add_argument(
        "--system",
        dest="search_system",
        action="store_true",
    ...
    search_parser.add_argument(
        "--no-system", dest="search_system", action="store_false", help="Exclude system notes/reports"
    )
    search_parser.add_argument(
        "--attachments",
        dest="search_attachments",
        action="store_true",
        default=True,
    ...
    search_parser.add_argument(
        "-uo", "--user-only", dest="user_only", action="store_true", help="User notes + attachments only (no system)"
    )
    search_parser.add_argument(
        "-un",
        "--user-notes-only",
        dest="user_notes_only",
        action="store_true",
    ...
    # Agent filter
    search_parser.add_argument(
        "--agent", dest="agent_filter", help="Filter by agent (shows notes owned by OR delegated to this agent)"
    )
    ...
    search_parser.add_argument("--offset", type=int, default=0, help="Skip first N results (for pagination)")
    ...
    search_parser.add_argument(
        "--mime-type", dest="mime_type", help="Comma-separated MIME types: text/markdown,application/pdf"
    )

    # Tag filters
    search_parser.add_argument(
        "--tags", dest="tags", help="Comma-separated tags to require (AND logic, e.g., --tags twitter,important)"
    )
    search_parser.add_argument(
        "--exclude-tags",
        dest="exclude_tags",
    ...
    read_parser.add_argument(
        "--limit", type=int, default=None, help="Maximum notes (default: 10 for llm/json, 100 for snippet/summary)"
    )
    read_parser.add_argument(
        "--format",
        choices=["json", "llm", "snippet", "summary"],
        default=None,
    ...
    read_parser.add_argument(
        "--processed-status",
        choices=["unprocessed", "all", "processed"],
        default="unprocessed",
        help="Which notes to retrieve (default: unprocessed)",
    )
    read_parser.add_argument("--all", action="store_true", help="Get all notes (shortcut for --processed-status all)")
    read_parser.add_argument(
        "--note-type", choices=["user", "system", "all"], default="user", help="Filter by note type (default: user)"
    )
    ...
    read_parser.add_argument("--from", dest="from_days", type=float, help="Days back to start (e.g., --from 7)")
    read_parser.add_argument(
        "--to", dest="to_days", type=float, default=0.0, help="Days back to end (default: 0.0 = now)"
    )
    read_parser.add_argument("--days", dest="days_back", type=float, help="Last N days (shortcut for --from N --to 0)")
    read_parser.add_argument("--recent", action="store_true", help="Last day (shortcut for --from 1)")
    read_parser.add_argument("--week", action="store_true", help="Last week (shortcut for --from 7)")
    read_parser.add_argument("--forever", action="store_true", help="All time (no time limit)")
    ...
    read_parser.add_argument("--with-signed-urls", action="store_true", help="Include public shareable URLs for notes")

    # Mark notes command
    mark_parser = notetaker_subparsers.add_parser(
        "mark", help="Mark or unmark notes as processed, exported, and/or deleted"
    )
    ...
    mark_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for concise output)"
    )
    ...
    delete_parser.add_argument(
        "unique_ids", nargs="+", help="Unique IDs of notes to delete (minimum 2 characters each)"
    )
    delete_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for concise output)"
    )
    ...
    undelete_parser.add_argument(
        "unique_ids", nargs="+", help="Unique IDs of notes to undelete (minimum 2 characters each)"
    )
    undelete_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for concise output)"
    )
    ...
    update_parser = notetaker_subparsers.add_parser(
        "update", help="Bulk update notes (tags, summary, metadata, delegates)"
    )
    update_parser.add_argument("unique_ids", nargs="+", help="Unique IDs of notes (minimum 2 characters each)")
    # Tags
    update_parser.add_argument(
        "--tags-set", dest="tags_set", type=str, help="Replace all tags (comma-separated: a,b,c)"
    )
    update_parser.add_argument(
        "--tags-add", dest="tags_add", type=str, help="Add tags (comma-separated: new_tag1,new_tag2)"
    )
    update_parser.add_argument(
        "--tags-remove", dest="tags_remove", type=str, help="Remove tags (comma-separated: old_tag)"
    )
    # Summary
    update_parser.add_argument("--summary", dest="summary", type=str, help='Set summary text (use "" to clear)')
    # Metadata
    update_parser.add_argument(
        "--metadata-set", dest="metadata_set", type=str, help="Replace entire metadata (JSON string)"
    )
    update_parser.add_argument(
        "--metadata-merge", dest="metadata_merge", type=str, help="Merge keys into metadata (JSON string)"
    )
    update_parser.add_argument(
        "--metadata-remove-keys",
        dest="metadata_remove_keys",
        type=str,
        help="Remove keys from metadata (comma-separated)",
    )
    # Delegates
    update_parser.add_argument(
        "--delegates-add", dest="delegates_add", type=str, help="Add delegate agents (comma-separated: coach,wellness)"
    )
    update_parser.add_argument(
        "--delegates-remove", dest="delegates_remove", type=str, help="Remove delegate agents (comma-separated)"
    )
    update_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for concise output)"
    )
    ...
    delegate_parser = notetaker_subparsers.add_parser(
        "delegate", help="Manage note delegation (alias for update --delegates-*)"
    )
    ...
    delegate_parser.add_argument(
        "--add", dest="delegates_add", type=str, help="Add agents to delegates (comma-separated: coach,wellness,flat)"
    )
    delegate_parser.add_argument(
        "--remove", dest="delegates_remove", type=str, help="Remove agents from delegates (comma-separated)"
    )
    delegate_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for one-line output)"
    )
    ...
    savereport_parser.add_argument(
        "filename", nargs="?", default=None, help="Markdown file to save (reads from stdin if not provided)"
    )
    savereport_parser.add_argument(
        "--agent", dest="agent", help="Agent name (extracted from frontmatter if not provided)"
    )
    savereport_parser.add_argument(
        "--report-type", dest="report_type", default="user", help="Report type (default: user)"
    )
    savereport_parser.add_argument(
        "--date", dest="date", help="Report date ISO format (extracted from frontmatter if not provided)"
    )
    savereport_parser.add_argument(
        "--format", choices=["json", "llm"], default="llm", help="Output format (default: llm for concise output)"
    )
    ...
    clear_parser.add_argument("--directory", default="_notes", help="Directory to clear (default: _notes)")
    ...
    reply_parser.add_argument("--markdown", action="store_true", help="Treat text as markdown (auto-generates HTML)")
    reply_parser.add_argument(
        "--delay", type=int, default=5, help="Delay in minutes before sending (default: 5). Use 0 for immediate."
    )
    ...
    send_parser.add_argument("--markdown", action="store_true", help="Treat text as markdown (auto-generates HTML)")
