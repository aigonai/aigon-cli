...
def main():
    ...
    parser.add_argument("--config-file", help="Config file path (default: ~/.aigon or AIGON_CLI_CONFIG_FILE env var)")

    parser.add_argument("--url", default=None, help="REST API base URL (default: https://api.aigon.ai or from config)")

    parser.add_argument("--token", help="API authentication token (default: from AIGON_API_TOKEN env var)")

    parser.add_argument(
        "--format", choices=["json", "table"], default="table", help="Default output format (default: table)"
    )
