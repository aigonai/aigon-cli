...
def test_aigon_client_authentication_headers():
    ...
    with patch("aigon_cli.client.requests.get") as mock_get:
        ...
        assert client.headers == {"Authorization": "Bearer test_token_123", "Content-Type": "application/json"}
...
class TestSaveReportVisibility:
    ...
    def test_save_report_sends_visibility_in_body(self):
        ...
        with patch("aigon_cli.client.requests.get") as mock_get, patch("aigon_cli.client.requests.post") as mock_post:
            # Mock initialization
            ...
            client = AigonClient(api_token="test_token")
            client.save_report(content="# Test", event="hackathon", visible_to_participants=True)
    ...
    def test_save_report_admin_only(self):
        ...
        with patch("aigon_cli.client.requests.get") as mock_get, patch("aigon_cli.client.requests.post") as mock_post:
            ...
            client = AigonClient(api_token="test_token")
            client.save_report(content="# Test", event="hackathon", visible_to_participants=False)
...
class TestUpdateNotes:
    ...
    def test_update_notes_sends_patch_request(self):
        ...
        with patch("aigon_cli.client.requests.get") as mock_get, patch("aigon_cli.client.requests.patch") as mock_patch:
            mock_get_response = Mock()
            mock_get_response.status_code = 200
            mock_get.return_value = mock_get_response
            ...
            mock_patch_response.json.return_value = {"success": True, "batch_size": 1, "operations": ["tags_add"]}
    ...
    def test_update_notes_sends_metadata_operations(self):
        ...
        with patch("aigon_cli.client.requests.get") as mock_get, patch("aigon_cli.client.requests.patch") as mock_patch:
            ...
            client = AigonClient(base_url="http://test", api_token="test_token")
            client.update_notes(unique_ids=["abc"], metadata_merge={"key": "val"}, metadata_remove_keys=["old_key"])
    ...
    def test_update_notes_sends_delegates(self):
        ...
        with patch("aigon_cli.client.requests.get") as mock_get, patch("aigon_cli.client.requests.patch") as mock_patch:
            ...
            client = AigonClient(base_url="http://test", api_token="test_token")
            client.update_notes(unique_ids=["abc"], delegates_add=["coach"], delegates_remove=["wellness"])
