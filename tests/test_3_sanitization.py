...
class TestSanitizeNoteForOutput:
    ...
    def test_removes_numeric_id(self):
        ...
        note = {"id": 12345, "unique_id": "abc123def456", "content": "Test content", "content_type": "text"}
    ...
    def test_removes_user_id_pk_int(self):
        ...
        note = {"id": 123, "user_id_pk_int": 456, "unique_id": "abc123", "content": "Test"}
        result = _sanitize_note_for_output(note)
    ...
    def test_removes_agent_field(self):
        ...
        note = {"id": 123, "agent": "notetaker", "unique_id": "abc123", "content": "Test"}
        result = _sanitize_note_for_output(note)
    ...
    def test_does_not_modify_original(self):
        ...
        note = {"id": 123, "unique_id": "abc123", "content": "Test"}
        original_keys = set(note.keys())

        _sanitize_note_for_output(note)
...
class TestFormatNoteLLM:
    ...
    def test_includes_unique_id(self):
        ...
        note = {"unique_id": "abc123def456789", "content": "Test content", "content_type": "text"}
    ...
    def test_includes_content_type(self):
        ...
        note = {"unique_id": "abc123", "content": "Test", "content_type": "audio"}
        result = _format_note_llm(note)
...
class TestSearchNotesOutput:
    ...
    def test_search_llm_format_is_default(self, mock_client_class):
        ...
        # get_notes_by_ids returns full note data
        mock_client.get_notes_by_ids.return_value = [
            {"id": 123, "unique_id": "abc123def456", "content": "Test content", "content_type": "text"}
        ]
...
class TestRecentNotesOutput:
    ...
    def test_recent_json_output_sanitized(self, mock_client_class):
        ...
        mock_client.get_recent_notes.return_value = [
            {"id": 789, "unique_id": "xyz789", "content": "Recent note", "content_type": "text", "user_id_pk_int": 123}
        ]
...
class TestMarkNotesOutput:
    ...
    def test_mark_notes_llm_format_is_concise(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client.mark_notes.return_value = {"success": True, "batch_size": 3, "found_count": 3, "requested_count": 3}

        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            mark_notes(mock_client, unique_ids=["abc", "def", "ghi"], processed=True, output_format="llm")
            output = mock_stdout.getvalue()
    ...
    def test_mark_notes_json_format_full_details(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client.mark_notes.return_value = {"success": True, "batch_size": 2, "found_count": 2}

        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            mark_notes(mock_client, unique_ids=["abc", "def"], processed=True, output_format="json")
            output = mock_stdout.getvalue()
    ...
    def test_mark_notes_unprocessed_message(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client.mark_notes.return_value = {"success": True, "batch_size": 1}

        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            mark_notes(mock_client, unique_ids=["abc"], processed=False, output_format="llm")
            output = mock_stdout.getvalue()
    ...
    def test_mark_notes_exported_only(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client.mark_notes.return_value = {"success": True, "batch_size": 2}

        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            mark_notes(mock_client, unique_ids=["abc", "def"], exported=True, output_format="llm")
            output = mock_stdout.getvalue()
    ...
    def test_mark_notes_both_flags(self, mock_client_class):
        ...
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            mark_notes(mock_client, unique_ids=["abc"], processed=True, exported=True, output_format="llm")
            output = mock_stdout.getvalue()
...
class TestFileDBListOutput:
    ...
    def test_list_files_llm_is_default(self, mock_client_class):
        ...
        mock_client = MagicMock()
        mock_client.list_files.return_value = {"files": [{"basename": "test", "version": 1, "unique_id": "TEST123456"}]}
...
class TestFormatNoteSummary:
    ...
    def test_shows_placeholder_when_no_summary(self):
        ...
        note = {"unique_id": "abc123", "content": "Full content", "content_type": "text", "created_at": 1700000000}
        result = _format_note_summary(note)
